...
fn main() {
    ...
    for address_space in buffer.split("\n") {
        ...
        let start_address = match usize::from_str_radix(memory_range.next().expect("should also be there"), 16) {
            Ok(v) => v,
            Err(e) => {println!("error getting start_address, {}", e);continue;}
        };
        ...
        let locs = match scan(Cursor::new(_bytes), &pattern) {
            Ok(v) => v,
            Err(e) => {println!("error getting locations: {}", e);continue}
        };
        ...
        let offset = vec![0, 0x68, 0x68, 0x28];

        let final_address = resolve_offsets_to_final_address(locs[0], offset, handle);
